...
OUT = Path("ml_baseline"); OUT.mkdir(parents=True, exist_ok=True)

# Set this to "PM25", "PM10" or "NO2"
TARGET = "NO2"
...
def metrics(y, p, tag):
    # version-proof RMSE (no squared=False arg)
...
# prevent leakage: remove TARGET from extra predictors
extra = ["WS", "AT", "RH", "BP", "NO2", "O3"]
extra = [c for c in extra if c != TARGET]

# add lag & rolling features for target + allowed predictors
df = add_lag_roll(df, cols=[TARGET] + extra, lags=(1,3,6,24), rolls=(3,6,24))
...
feature_cols = [
    ...
    if c not in (["datetime","station","lat","lon"] + exclude_raw_targets)
]
